...
By default, the CLI listens on an ephemeral localhost port and sends a matching redirect URI such as `http://127.0.0.1:<port>/callback` to the identity provider. If your OIDC client must allowlist one fixed redirect URI, pass `--oidc-redirect-uri` during registration:
...
openshell gateway add https://gateway.example.com \
  ...
  --oidc-client-id openshell-cli \
  --oidc-audience openshell-cli \
  --oidc-redirect-uri http://127.0.0.1:8765/callback
```

The fixed redirect URI must use `http://` and include an explicit host, port, and path. Register the same URI with your identity provider. OpenShell stores it with the gateway metadata so `openshell gateway login` reuses the same redirect URI.
...
For OIDC gateways, `metadata.json` also stores the issuer, CLI client ID, optional audience, requested scopes, and optional fixed redirect URI. Treat `oidc_token.json` as a credential. OpenShell writes it with owner-only file permissions.
